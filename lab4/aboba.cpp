...
int main()
{
    try
    {
        ...
        else
        {
            ...
            {
                std::cout << std::endl;
                for (int j = 0;j < space+1; j++) // цикл для изначально пространства, до начала печати
                {
                    std::cout << " ";
                }
                for (int j = 0; j <= i-1; j++) // Печать i + 1 звезд
                {
                    std::cout << "* ";
                }
            ...
            for (int i = n; i > -1; i--) // запустить цикл до количества строк
            {
                std::cout << std::endl;
