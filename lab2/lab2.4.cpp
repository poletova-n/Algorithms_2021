...
int main() {
    ...
    if ((abs(hunter[0]- victim[0]) == abs(hunter[1] - victim[1])) or ((hunter[0] == victim[0]) or (hunter[1] == victim[1])))  {
        cout << "Ферзь съедает фигуру противника";
    }
    else {
        cout << "Ферзь не может съесть фигуру противника";
    }
