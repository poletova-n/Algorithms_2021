...
// 7 Инфиксный обход дерева (итеративный)
    ...
    void iterativeWideWalk() const {
        auto queue = new CircleQueue<Node<T> *>;
        queue->enQueue(root_);
        while (!queue->isEmpty()) {
            Node<T> *temp = queue->deQueue();
            std::cout << temp->key_ << " ";
            if (temp->left_ != nullptr) queue->enQueue(temp->left_);
            if (temp->right_ != nullptr) queue->enQueue(temp->right_);
        }
        delete queue;
        std::cout << std::endl;
    };
...
// 8.1 Инфиксный обход дерева (рекурсивный)
    ...
    bool compare(BinarySearchTree<T> *&other) {
        ...
        if (other->getCount() != this->getCount()) return false;

        auto *tempQueue = new CircleQueue<T>;
        auto *stack = new std::stack<Node<T> *>;
        ...
        while (temp != nullptr || !stack->empty()) {
            if (temp != nullptr) {
                ...
                temp = stack->top();
                stack->pop();
                if (tempQueue->deQueue() != temp->key_) return false;
                temp = temp->right_;
            }
        }
    ...
    void intersection(BinarySearchTree<T> *&other){
        ...
        while (temp != nullptr || !stack->empty()) {
            ...
            if (temp != nullptr) {
                ...
                temp = stack->top();
                stack->pop();
                T tempSys = tempQueue->deQueue();
                while( temp->key_ < tempSys){
                    temp = stack->top();
                    stack->pop();
                }
                if (tempSys == temp->key_) std::cout << temp->key_ << " ";
                temp = temp->right_;
            }
        }
