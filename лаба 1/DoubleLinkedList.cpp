...
{
	if (this != &right) {
		 // temp копия right операнда
		swap(right);        // теперь this копия right, а temp копия this
...
{
	if (count_ != list.count_) //сравниваем число элементов
	{
		return 0;
	}
	Node* x = head_;
	...
	while (x != nullptr && y != nullptr) //сравниваем элементы начиная с начала
	{
		if (x->item_ == y->item_)
		{
			x = x->next_;
...
{
	if (tail_ != nullptr) // если есть последний элемен, то его записываем в предпослениий
	{
		x->prev_ = tail_; 
		tail_->next_ = x; 
	}
...
bool DoubleLinkedList::deleteTail()
{
	if (tail_ == nullptr)
	{
		return 0; // если список пуст возращаяем false
	}
	deleteNode(tail_);
	return 1;  
}
...
//Добавление в хвост исходного списка
